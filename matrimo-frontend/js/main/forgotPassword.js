...
document.getElementById("actionBtn").addEventListener("click", async function(event) {
    ...
    if (currentStep === 'sendOtp') {
        ...
        const response = await fetch("https://backend-green-seven-44.vercel.app/api/user/user/forgot-password", {
            method: "POST",
       ...
       showLoader()
        const response = await fetch("https://backend-green-seven-44.vercel.app/api/user/user/reset-password", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
